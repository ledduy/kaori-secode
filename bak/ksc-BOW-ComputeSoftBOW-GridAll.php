...
$szTmpDir = sprintf("%s/%s.bow.computeSoftHistWithGridAll/%s/%s-%s-%d-%d", $szLocalTmpDir, $szScriptBaseName, $szPatName, $szInputFeatureExt, $szInputRawFeatureExt, $nStartID, $nEndID);
makeDir($szTmpDir);
...
computeSoftBOWHistogramWithGridForOnePat($szTmpDir, $szRootFeatureInputDir, $szRootMetaDataDir, $szFPVideoListFN, $szInputFeatureExt, $szInputRawFeatureExt, $nMaxCodeBookSize, $nStartID, $nEndID);
...
function computeSoftWeightingHistogramWithGrid($szFPKeyFrameListFN, $szFPOutputFN, $szFPInputLabelFN, $szInputRawFeatureDir, $szInputRawFeatureExt, $szLocalDir, $nNumRows = 2, $nNumCols = 2, $nMaxCodeBookSize = 2000) // for concatenating feature vectors of sub regions
{
...
function computeSoftBOWHistogramWithGridForOneVideoProgram($szLocalDir, $szFPKeyFrameListFN, $szRootFeatureInputDir, $szVideoPath, $szVideoID, $szInputFeatureExt, $szInputRawFeatureExt, $nMaxCodeBookSize = 2000)
{
    $arGridList = array(
        // 4 => 4,
        3 => 1,
    ...
    foreach ($arGridList as $nNumRows => $nNumCols)
    {
        ...
        computeSoftWeightingHistogramWithGrid($szFPKeyFrameListFN, $szFPLocalOutputFN, $szFPLocalInputFN, $szInputRawFeatureDir, $szInputRawFeatureExt, $szLocalDir, $nNumRows, $nNumCols, $nMaxCodeBookSize);
        
        $szFPTarLocalOutputFN = sprintf("%s.tar.gz", $szFPLocalOutputFN);
        $szCmdLine = sprintf("tar -cvzf %s -C %s %s", $szFPTarLocalOutputFN, $szLocalDir, $szOutputCoreName);
        execSysCmd($szCmdLine);
...
function computeSoftBOWHistogramWithGridForOnePat($szLocalDir, $szRootFeatureInputDir, $szRootMetaDataDir, $szFPVideoListFN, $szInputFeatureExt, $szInputRawFeatureExt, $nMaxCodeBookSize = 2000, $nStartID = -1, $nEndID = -1)
{
    ...
    for ($i = $nStartID; $i < $nEndID; $i ++)
    {
        ...
        computeSoftBOWHistogramWithGridForOneVideoProgram($szLocalDir2, $szFPKeyFrameListFN, $szRootFeatureInputDir, $szVideoPath, $szVideoID, $szInputFeatureExt, $szInputRawFeatureExt, $nMaxCodeBookSize);
