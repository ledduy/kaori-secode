...
/*
 * Input is xml file (e.g. iacc.2.A.collection.xml) provided by TRECVID: + collection: mapping between videoID and videoName, patName Output is a set of files for NII-SECODE + video.trecvid.lst: all in one file, mapping between videoID, videoName, and path + tv200x.lst: mapping between videoID, videoName, and path for given year. + tv2012.test.lstx --> extend lst file by adding duration, framecount, frame rate
 */
...
foreach ($arCode as $nTVYear => $szCode)
{
    ...
    foreach ($arVideoList as $szLine)
    {
        ...
        $szOutput = sprintf("%s #$# %0.2f #$# %s #$# %0.4f #$# %0.4f", $szLine, $fFrameRate, $nFrameCount, $nDuration, $fBitRate);
        $arOutput[] = $szOutput;
        printf("### %s\n", $szOutput);
    }
 ...
 *
 * @param $szCode -->
 *            used in path = tv2013/test.<Code ~ iacc.2.A> - special for tv2013
 */
 ...
 * </VideoFile>
 */
function generateMetadataForOneYearTRECVID($szRootMetaDataOutputDir, $szRootMetaDataInputDir, $nTVYear = "2013", $szCode = "iacc.2.A")
{
    global $gszDelim;
