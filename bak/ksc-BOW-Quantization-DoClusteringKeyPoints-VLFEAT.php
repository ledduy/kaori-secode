...
/*
 * 1. $cd /net/per900b/raid0/ledduy/bin/vlfeat-0.9.8 2. $run('toolbox/vl_setup') 3. Import to var 'data' 4. K=500; 5. [C,A] = vl_ikmeans(uint8(data'),K,'method', 'elkan') ; // convert to unit8, and transpose 6. dlmwrite('output.csv', C'); // transpose C 7. dlmwrite('output.idx', A');
 */
...
foreach ($arPatList as $szPatName)
{
    ...
    foreach ($arFeatureList as $szFeatureExt)
    {
        ...
        $szInputDir = sprintf("%s/bow.codebook.%s.%s/%s", $szRootFeatureDir, $szTrialName, $szPatName, $szFeatureExt);
...
/*
 * % Do clustering using VLFEAT-matlab % This script must be put in vlfeat dir % szFPInputFN - input file name in csv format % szFPCentroidOutputFN - centroids of output clusters % szFPIMemOutputFN - cluster assignment % Using integer k-means % run('toolbox/vl_setup'); --> must call to init env % 1M keypoints --> 3GB memory % Written by Duy-Dinh Le % Last update: Feb 08, 2011 function nsc_BOW_DoClusteringKeyPoints_VLFEAT_TV10(szFPCentroidOutputFN, szFPIMemOutputFN, szFPInputFN, nNumClusters, szMethod) run('toolbox/vl_setup'); % init env fprintf(1, 'Loading csv data file ...\n'); data = csvread(szFPInputFN); fprintf(1, 'Performing k-means [%u] clusters with method [%s]...\n', nNumClusters, szMethod); % [C,A] = vl_ikmeans(uint8(data'),nNumClusters,'method', szMethod) ; % convert to unit8, and transpose [C,A] = vl_kmeans(data',nNumClusters,'method', szMethod) ; % transpose fprintf(1, 'Saving output data...\n'); dlmwrite(szFPCentroidOutputFN, C'); % transpose C dlmwrite(szFPIMemOutputFN, A'); quit; % quit matlab since it is used to run within PHP end
 */
?>