    ...
    foreach ($arDestPatList as $szDestPatName)
    {
        ...
        foreach ($arFeatureList as $szFeatureExt)
        {
            ...
            for ($j = 0; $j < $nMaxVideosPerPat; $j += $nNumVideosPerHost)
            {
                ...
                $szParam = sprintf("%s %s %s %s %s %s", $szSourcePatName, $szFeatureExt, $szFPPatName, $nSamplingInterval, $nStart, $nEnd);
            ...
            $szFPOutputFN = sprintf("%s/runme.qsub.%s.%s.%s.sh", $szScriptOutputDir, $szCoreScriptName, $szFPPatName, $szFeatureExt); // specific for one set of data
