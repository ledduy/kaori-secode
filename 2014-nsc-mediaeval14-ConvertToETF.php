...
$szProjectKF = $argv[3];
$szGroupSet = $argv[4];
$szConceptName = $argv[5];
$szThreshold = floatval($argv[6]);
...
foreach ($arrFeatures as $szFeature)
{
		...
		$szRootOutResult =  sprintf("/net/per610a/export/das11f/ledduy/mediaeval-vsd-2014/submission/keyframe-5/%s/%s",$szPrjName,$szFeature);
		if (!file_exists($szRootOutResult))
		{
			makeDir($szRootOutResult);
		}
		$arrRankList = array();
		...
		if ( file_exists($szViolentscenes) && (filesize($szViolentscenes)>0) )
		{
			...
			$strcmd = sprintf("cp %s %s/%s.etf",$szFileETF,$szRootOutResult,$szRootAtt);
			execSysCmd ($strcmd);
			// end of copy
			...
			printf("\nConvert successfull! %s/%s.etf\n<br>",$szRootInResult,$szRootAtt);
		}	
