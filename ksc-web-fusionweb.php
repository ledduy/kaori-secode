...
if($nAction == 2)  // Let user pick the ModelFeatureConfig
{
	...
	printf("<H2>Config name:<INPUT TYPE=TEXT NAME='vCfgName' VALUE='%s' SIZE=50></H2>",$szFilter);
...
if($nAction == 3)  // Let user pick the ModelFeatureConfig
{
	...
	{
		$strtmp = sprintf("%s #$# %d\n",$arRunList[$i],$arWeight[$arFeaturesID[$arRunList[$i]]]);
		printf("%s<br>\n",$strtmp);
		fwrite($fcfg,$strtmp);
	} 
	fclose($fcfg);
	
	$strcmd = sprintf("chmod -R 777 %s",$szFusionCfgName);
	execSysCmd ($strcmd);
