...
/*
 * $loadFlag is 1 if previously-saved temporary files are to be loaded and reused whenever they might exist, and 0 if temporary files are not to be reused;
 */
$arGreedyRSCEnv["loadFlag"] = 1;

/*
 * $saveFlag is 1 if temporary files are to be retained after program execution ends, and 0 if temporary files are not to be retained;
 */
$arGreedyRSCEnv["saveFlag"] = 1;

/*
 * $sashDeg is the internal node degree parameter for the SASH approximate search structure (recommended value: 4);
 */
$arGreedyRSCEnv["sashDeg"] = 4;

/*
 * $scaleFactor is the time-accuracy trade-off parameter for the SASH approximate search structure (recommended value: 1.5); 0 is exact search SPEED RELATED
 */
$arGreedyRSCEnv["scaleFactor"] = 0;

/*
 * $minCluSig is the minimum threshold on the normalized squared Z-statistic (significance) score for the cluster - for a perfectly well-associated cluster, this number would equal the cluster size (recommended value: 4.0); ACCURACY RELATED
 */
$arGreedyRSCEnv["minCluSig"] = 4;

/*
 * $minMemSig is the minimum threshold on the individual set correlation between a cluster and each of its members, also used as a minimum limit on the square of the average set correlation between a cluster and its members (recommended values: 0.1, or 0.15 when main memory is limited); lower value --> noisier cluster ACCURACY RELATED
 */
$arGreedyRSCEnv["minMemSig"] = 0.1;

/*
 * $maxEdgeSig is the maximum inter-cluster correlation value for two clusters to be declared distinct (recommended value: 0.5); Increasing the value of parameter <maxEdgSig> also results in the generation of a larger number of clusters.
 */
$arGreedyRSCEnv["maxEdgeSig"] = 0.5;

/*
 * $minEdgeSig is the minimum inter-cluster correlation value necessary for two clusters to be declared related (recommended value: 0.1); controls only the reporting of relationships between pairs of clusters – it has no effect on the cluster generation process itself.
 */
...
/*
 * $annotType is the file type for the input data (either ‘img’ for images, ‘html’ for a link in HTML format, or ‘txt’ for plain txt);
 */
$arGreedyRSCEnv["annotType"] = "img";

/*
 * $annotFlag is 1 if annotations have been provided in ANN files, and 0 if default item labels should be used;
 */
...
function runGreedyRSCClustering($szDataName, $szInputDirPath, $szTmpDirPath, $szOutputDirPath, $nMinClusSize)
{
    ...
    /*
     * Usage: GreedyRSC <dataFileName> <fileNameExt> <inputDirPath> <tempDirPath> <outputDirPath> <loadFlag> <saveFlag> <sashDeg> <scaleFactor> <minCluSig> <minMemSig> <maxEdgeSig> <minEdgeSig> <seed>
     */
    ...
    $szCmdLine = sprintf("%s %s %s %s %s %s %f %f %f %f %f %f %f %f %f", $szGreedyRSCClusteringApp, $szDataName, $fileNameExt, $szInputDirPath, $szTmpDirPath, $szOutputDirPath, $loadFlag, $saveFlag, $sashDeg, $scaleFactor, $minCluSig, $minMemSig, $maxEdgeSig, $minEdgeSig, $seed);
...
function runGreedyRSCPartition($szDataName, $szClusDirPath, $szOutputDirPath, $nMinClusSize, $fPartitionThreshold)
{
    ...
    /*
     * Usage: GreedyRSCPartition <fileNamePrefix> <clustDirPath> <partitDirPath> <minClustSize> <partition_threshold>
     */
    ...
    $szCmdLine = sprintf("%s %s %s %s %f %f", $szGreedyRSCPartitionApp, $szDataName, $szClusDirPath, $szOutputDirPath, $nMinClusSize, $fPartitionThreshold * 10000);
    printf("Command: [%s]\n", $szCmdLine);
...
function runAnnotator($szDataName, $szAnnDirPath, $szClusDirPath, $szOutputDirPath, $szRelPath, $szFileExt)
{
    ...
    $szCmdLine = sprintf("%s %s %s %s %s %s %d %d %s %s", $szGreedyRSCAnnotatorApp, $szDataName, $annotType, $szAnnDirPath, $szClusDirPath, $szOutputDirPath, $annotFlag, $seed, $szRelPath, $szFileExt);
...
function runSashTester($szDataName, $szInputDirPath, $szOutputDirPath, $fStartScaleFactor = 0.125, $nNumQueries = 100, $nNumRepeats = 10)
{
    ...
    /*
     * Usage: SashTester <fileNamePrefix> <fileNameExt> <inputDirPath> <outputDirPath> <sashDeg> <scaleFactor> <numQueries> <trialsPerQuery> <seed>
     */
    ...
    $szCmdLine = sprintf("%s %s %s %s %s %f %f %n %n %n", $szGreedyRSCSashTesterApp, $szDataName, $fileNameExt, $szInputDirPath, $szOutputDirPath, $sashDeg, $scaleFactor, $nNumQueries, $nNumRepeats, $seed);
 ...
 */
function viewMembersOfOneClusterFromMem(&$arCluster, &$arAnnList, $nClusterID, $szRelPath, $szVideoPath, $szImgExt, $szViewClusterMemberScript, $szAddParams = "", $nImgWidth = 80, $nNumSampleMembersPerCluster = -1)
{
    ...
    for ($j = 0; $j < $nNumSamplesToShow; $j ++)
    {
        ...
        $szParams = sprintf("vVideoPath=%s&vRelPath=%s&vImgName=%s&vClusterID=%s&vMemberID=%s", $szVideoPath, $szRelPath, $szImgName, $nClusterID, $nMemberID);
...
function loadMemFileWithThreshold(&$arRSCMemList, $szDataName, $szInputDir, $nMaxSize = -1, $nNumChunks = -1, $nNumBlocksPerChunk = -1, $fMemThreshold = 0.1, $fMinSNN = 4, $fMinRelSNNSize = 0.25)
{
    global $gszMaxClusterSize;
    global $gszMaxChunks;
    global $gszMaxBlocksPerChunk;
...
function loadClusterData(&$arRSCMemList, &$arAnnList, $arFeatureList, $szOrigDataName, $szOrigClusterDir, $szOrigDataDir, $fPartThreshold, $nNumChunks, $nNumBlocksPerChunk, $nMaxSize, $fMemThreshold, $fMinSNN, $fMinRelSNNSize)
{
    ...
    for ($i = 0; $i < $nNumFeatures; $i ++)
    {
        ...
        $arAnnListTmp = array();
        loadMemFileWithThreshold($arRSCMemListTmp, $szDataName, $szClusterDir, $nMaxSize, $nNumChunks, $nNumBlocksPerChunk, $fMemThreshold, $fMinSNN, $fMinRelSNNSize);
...
/*
 * The SASH parameters <sashDeg> and <scaleFactor> together control the time-accuracy trade-off for the relevant set generation phase of the clustering process. For most data sets, a default fixed value of <sashDeg>=4 is recommended, with <scaleFactor> being set to between 1.0 (faster, lower accuracy) and 2.0 (slower, higher accuracy). However, for protein sequence clustering where data vector coordinates are based on BLAST E-values with a reference subset of sequences, better performance was achieved with a higher SASH degree of <sashDeg>=16 and a corresponding lower scale factor of <scaleFactor>=0.125. As a general recommendation, the average SASH recall accuracy should be at least 60% when searching for 120 neighbors of query items drawn from the data set (“query-by-example”) – higher accuracies are of course preferable! Users should consider using the SashTester utility program to explicitly test the SASH performance beforehand if accuracy or running time is of particular concern. The parameters <minCluSig> and <minMemSig> strongly influence the number of candidates generated in the course of the clustering, as well as the final number of clusters produced. Setting the parameters to values lower than those recommended above can lead to a blowout in main-memory usage when clustering large data sets. Increasing the value of parameter <maxEdgSig> also results in the generation of a larger number of clusters. Parameter <minEdgSig> controls only the reporting of relationships between pairs of clusters – it has no effect on the cluster generation process itself.
 */
?>
