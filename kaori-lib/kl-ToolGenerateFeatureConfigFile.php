...
for ($i = 0; $i < $nNumGridPoints; $i ++)
{
    ...
    for ($j = 0; $j < $nNumColorTypes; $j ++)
    {
        ...
        for ($k = 0; $k < $nNumCHBinTypes; $k ++)
        {
            ...
            $arOutputNameList[] = sprintf("%s #$# %s", $szFeatureExt, $szOutputName);
            $szFPOutputFN = sprintf("%s/%s", $szOutputDir, $szOutputName);
            generateCHFeatureConfigFile($szFPOutputFN, $szColorType, $nNumRows, $nNumCols, $nNumBins, $nNumBins, $nNumBins);
        }
...
/*
 * $szFeatureListName = "BaselineFeatureConfig-tv2009"; $szFPOutputFN = sprintf("%s/%s.lst", $szOutputDir, $szFeatureListName); saveDataFromMem2File($arOutputNameList, $szFPOutputFN, "wt");
 */
