...
if($argc != 7)
{
	...
	printf("Usage: %s %s %s %s %s %s %s \n", $argv[0], 
				$szExpName, $szFPRunConfigFN, $nStartConceptID, $nEndConceptID, $nStartVideo, $nEndVideo);
	exit();
}
...
/*
 ...
 run_id_001_feature #$# nsc.cCV_HSV.g5.q3.g_cm
 run_id_001_weight #$# 1
 run_id_001_norm #$# 1 #$# require normalize score before fusion
 */
...
for($i=$nStart; $i<$nEnd; $i++)
{
	...
	$szExpScoreRunConceptDir =  sprintf("%s/%s",
	$szExpResultRunDir, $szConceptName);
	makeDir($szExpScoreRunConceptDir);

	$arConceptDirList = array();
	foreach($arDirList as $szKey => $szDirName)
	{
		$szFusedRunConceptDir =  sprintf("%s/%s",
		$szDirName, $szConceptName);
		$arConceptDirList[$szKey] = $szFusedRunConceptDir;
	}
	fusionKeyFrameScoresForOneConcept($szExpScoreRunConceptDir, $szFeatureExt,
	$arVideoList, $arConceptDirList, $arWeightList, $arNormOptionList, $arFeatureExtList, $nStartVideo, $nEndVideo
	, $arMinThresholdList);
}
...
function fusionKeyFrameScores($szFPOutputFN, $arFileList, $arWeightList, $arNormOptionList
		, $arMinThresholdList)
{
	$arScoreList = array();
...
function fusionKeyFrameScoresForOneConcept($szOutputDir, $szOutputFeatureExt,
		&$arVideoList, &$arDirList, &$arWeightList, &$arNormOptionList, &$arFeatureExtList, $nStartVideo, $nEndVideo
		, &$arMinThresholdList)
{
