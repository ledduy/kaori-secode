...
# DuyLe - Not use gnuplot    assert os.path.exists(gnuplot_exe),"gnuplot executable not found"
    assert os.path.exists(dataset_pathname),"dataset not found"
# Duy Le    gnuplot = Popen(gnuplot_exe,stdin = PIPE).stdin
...
def main():
    ...
    for line in jobs:
        ...
        # DuyLe - do not output to gnuplot - redraw(db,[best_c1, best_g1, best_rate])
        # DuyLe - do not output to gnuplot - redraw(db,[best_c1, best_g1, best_rate],True)
