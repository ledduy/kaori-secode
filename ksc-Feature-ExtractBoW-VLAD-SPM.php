...
/**
 * 		@file 	ksc-Feature-ExtractBoW-VLAD-SPM.php
* 		@brief 	Extract BoW-VLAD Features - AllInOne Step.
...
$arNormMethod = array(
    1 => "L2",
    2 => "SquareRoot",
    3 => "NormComp", // NormalizeComponents
    4 => "NormMass", //NormalizeMass
);
...
$szVLADModelDir1 = $szRootCentroidDir1;

// copy VLAD data to /local/ledduy
$szRootCentroidDir = sprintf("%s", $szTmpDir);
$szVLADModelDir = sprintf("%s/gmm-%d-%d", $szRootCentroidDir, $nStartID, $nEndID);
makeDir($szVLADModelDir);

// / !!! IMPORTANT
$szCmdLine = sprintf("cp %s/data/*VLAD*.mat %s", $szVLADModelDir1, $szVLADModelDir);
execSysCmd($szCmdLine);

$szVLADModelName = sprintf("%s.%s.%s.VLADModel", $szTrialName, $szPatName, $szInputRawFeatureExt);
...
$szFPVLADModelFN = sprintf("%s/%s.mat", $szVLADModelDir, $szVLADModelName);

if (! file_exists($szFPVLADModelFN))
{
    printf("### Serious Error - VLAD model not found [%s]\n", $szFPVLADModelFN);
    exit();
}
...
computeFisherVectorBOWHistogramWithGridForOnePat($szTmpDir, $szFeatureConfigParam, $szRootFeatureOutputDir, $szRootKeyFrameDir, $szRootMetaDataDir, $szVLADModelDir, $szVLADModelName, $szPrefixAnn, $szTargetPatName, $szInputRawFeatureExt, $szBOWFeatureExt, $nMaxCodeBookSize, $nStartID, $nEndID);

// clean up
$szCmdLine = sprintf("rm -rf %s", $szVLADModelDir);
execSysCmd($szCmdLine);
...
// ///////////////////////////// FUNCTIONS //////////////////////////////
function computeFisherVectorBOWHistogramWithGridForOnePat($szLocalDir, $szFeatureConfigParam, $szRootFeatureOutputDir, $szRootKeyFrameDir, $szRootMetaDataDir, $szVLADModelDir, $szVLADModelName, $szPrefixAnn, $szPatName, $szInputRawFeatureExt, $szBOWFeatureExt, $nMaxCodeBookSize = 2000, $nStartVideoID = -1, $nEndVideoID = -1)
{
    ...
    for ($i = $nStartVideoID; $i < $nEndVideoID; $i ++)
    {
        ...
        // TO BE UPDATE
        computeVLADBOWHistogramWithGridForOneVideoProgram($szLocalFeatureDir, $szRootFeatureOutputDir, $szFPKeyFrameListFN, $szVLADModelDir, $szVLADModelName, $szPrefixAnn, $szVideoPath, $szVideoID, $szInputRawFeatureExt, $szBOWFeatureExt, $nMaxCodeBookSize);
...
function computeVLADBOWHistogramWithGridForOneVideoProgram($szLocalFeatureDir, $szRootFeatureOutputDir, $szFPKeyFrameListFN, $szVLADModelDir, $szVLADModelName, $szPrefixAnn, $szVideoPath, $szVideoID, $szInputRawFeatureExt, $szBOWFeatureExt, $nMaxCodeBookSize = 2000)
{
    $arGridList = array(
        // 3 => 1,
        // 2 => 2,
    ...
    foreach ($arGridList as $nNumRows => $nNumCols)
    {
        ...
        computeSoftWeightingHistogramWithGrid($szFPVLADModelFN, $szFPKeyFrameListFN, $szFPLocalOutputFN, $szLocalFeatureDir, $szInputRawFeatureExt, $nNumRows, $nNumCols, $nMaxCodeBookSize);
...
function computeSoftWeightingHistogramWithGrid($szFPVLADModelFN, $szFPKeyFrameListFN, $szFPOutputFN, $szLocalFeatureDir, $szInputRawFeatureExt, $nNumRows = 2, $nNumCols = 2, $nMaxCodeBookSize = 2000) // for concatenating feature vectors of sub regions
{
    global $nNormMethod;
    global $arNormMethod;
    ...
    foreach ($arKeyFrameSizeLUT as $szKeyFrameID => $arSize)
    {
        ...
        foreach ($arInvPointLUT as $nRectIndex => $arPointList)
        {
            ...
            $szParam = sprintf("'%s', '%s', %d", $szFPVLADModelFN, $szFPCSVFeatureFN, $nNormMethod);
            $szCmdLine = sprintf("matlab -nodisplay -nojvm -r \"ksc_VLAD_PerformVLADEncoding_VLFEAT(%s)\" ", $szParam);
            printf("Command: [%s]\n", $szCmdLine);
            $arCmdLine[] = $szCmdLine;
            
            $szPrefix = sprintf("ksc_FV_PerformVLADEncoding_VLFEAT_%s", $szInputRawFeatureExt);
            ...
            $szFPVLADEncFN = sprintf("%s.fve", $szFPCSVFeatureFN);
            
            if (file_exists($szFPVLADEncFN))
            {
                $nNumRows = loadListFile($arRawList, $szFPVLADEncFN);
