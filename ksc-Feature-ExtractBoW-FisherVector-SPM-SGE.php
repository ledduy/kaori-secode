...
/**
 * 		@file 	ksc-Feature-ExtractBoW-FisherVector-SPM-SGE.php
 * 		@brief 	Generate jobs for SGE to computing BOW - Fisher Vector - SPM.
...
$szCoreScriptName = "ksc-Feature-ExtractBoW-FisherVector-SPM"; // *** CHANGED ***
...
if ($argc != 4)
{
    printf("Usage: %s <SourcePatName> <DestPatName> <NumFVClusters>\n", $argv[0]);
    printf("Usage: %s %s %s %s\n", $argv[0], $szSourcePatName, $szDestPatName, $nNumFVClusters);
    exit();
}
...
{
    $szScriptOutputDir = sprintf("%s/bow.%s.%s/%s-%sNorm", $szRootScriptOutputDir, $szTrialName, $szSourcePatName, $szFeatureExt, $arNormMethod[$nNormMethod]);
    ...
    for ($j = 0; $j < $nMaxVideosPerPat; $j += $nNumVideosPerHost)
    {
        ...
        $szParam = sprintf("%s %s %s %s %s %s %s", $szSourcePatName, $szDestPatName, $szFeatureExt, $nNumFVClusters, $nNormMethod, $nStart, $nEnd);
    ...
    $szFPOutputFN = sprintf("%s/runme.qsub.%s.%s.%s-%sNorm.sh", $szScriptOutputDir, $szCoreScriptName, $szDestPatName, $szFeatureExt, $arNormMethod[$nNormMethod]); // specific for one set of data
