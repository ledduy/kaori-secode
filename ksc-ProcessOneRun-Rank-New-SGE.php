...
/**
 * 		@file 	ksc-ProcessOneRun-Rank-New-SGE.php
 * 		@brief 	Rank One Run.
...
$szCoreScriptName = "ksc-ProcessOneRun-Rank-New"; // *** CHANGED ***
...
foreach ($arRunList as $szRunID)
{
    ...
    for ($j = 0; $j < $nMaxConcepts; $j += $nNumConceptsPerHost)
    {
        ...
        $szParam = sprintf("%s %s %s %s", $szTestConfigName, $szRunID, $nStart, $nEnd);
...
foreach($arHostList as $szHostName => $nMaxJobs)
{
	...
	for($i=$nStart; $i<$nEnd; $i++)
	{
		...
		for ($j = 0; $j < $nMaxConcepts; $j += $nNumConceptsPerHost)
		{
			...
			$szParam = sprintf("%s %s %s %s", $szTestConfigName, $szRunID, $nStartConcept, $nEndConcept);
			//printf($szParam);
