...
/*
 * Convert metadata of trecvid to nii-secode metadata 1. input: collection.xml --> output: tvxxx.video.id-name.map --> mapping between video id (keyframe dir) and video name (file mpg) 2. input: 1.xml --> output:
 */
...
/**
 *
 * @param $szFPCollectionXMLFN Output:
 *            videoID #$# videoName #$# path2videoID
 */
function parseOneCollectionXMLFile($szFPCollectionXMLFN)
{
    // adding for processing tv2010
    $arFileExtMapping = array(
        "MPEG-1" => ".mpg",
        "MPEG-4" => ".mp4",
        "MPEG-2" => ".mpg"
    );
            ...
            for ($k = 0; $k < $nNumVideos; $k ++)
            {
                ...
                for ($l = 0; $l < $nNumShots; $l ++)
                {
                    ...
                    for ($m = 0; $m < $nNumKeyFrames; $m ++)
                    {
                        ...
                        $szAnn = sprintf("%s #$# %s #$# %s #$# %s #$# %s #$# %s #$# %s", $szAnnOrg, $szAnnotatorName, $szConceptName, $szVideoID, $szShotID, $szKeyFrameID, $szShortLabel);
                        $arOutputListx[] = $szAnn;
...
/**
 * arShotList --> list of shots in plain list, e.g.
 * arShotList[] = TRECVID2005_1.shot1_12
