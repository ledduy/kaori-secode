...
// Only COLORSIFT descriptors are SUPPORTED
// Because Code for raw feature extraction is FIXED to COLORSIFT (Aug30, 2014)
...
$szSrcPatName = "devel2011-new"; // must be a member of $arPatList - CHANGED FOR VSD14
...
// *** CHANGED ***
$nAveShotPerVideo = 10; // CHANGED FOR VSD14 one videoID-new = 1,250KF, 1 shot = 125KF (5secs) --> 10shots/videoID-new
...
$szSrcPatName = trim($argv[1]);
$szRawFeatureExt = trim($argv[2]);

if(!isset($arFeatureParamConfigList[$szRawFeatureExt]))
{
    print_r($arFeatureParamConfigList);
    exit("Feature ext [$szRawFeatureExt] is not supported. Check arFeatureParamConfigList\n");
}
...
$szFPBOWConfigFN = sprintf("%s/%s.%s.cfg", $szOutputDir, $szSrcPatName, $szRawFeatureExt);
saveDataFromMem2File($arConfigOutput, $szFPBOWConfigFN);

$szFPLogFN = sprintf("%s/ksc-BOW-Quantization-SelectKeypointsForClustering-%s.log", $szOutputDir, $szRawFeatureExt); // *** CHANGED ***
