...
/*
 * sys_id #$# trial-1-s sys_desc #$# Test svf format and new combination feature_ext #$# nsc.cCV_GRAY.g6.q59.g_lbp kernel #$# 5 #$# Chi-Square RBF num_dup_sys #$# 3 max_kf_test #$# 5 devel_pat #$# tv2010.devel-nist.lst test_pat #$# tv2010.test.lst sub_ann #$# lig.iacc1.tv2010
 */
...
/*
 * sys_id #$# trial-1-s sys_desc #$# Test svf format and new combination feature_ext #$# nsc.cCV_GRAY.g6.q59.g_lbp kernel #$# 5 #$# Chi-Square RBF num_dup_sys #$# 3 max_kf_test #$# 5 devel_pat #$# tv2010.devel-nist.lst test_pat #$# tv2010.test.lst sub_ann #$# lig.iacc1.tv2010 use_keyframe_annotation #$# 1 svm_train_use_prob_output #$# 1 svm_train_pos_weight #$# -1 #$# Auto svm_train_neg_weight #$# -1 #$# Auto svm_grid_start_C #$# 0 svm_grid_end_C #$# 4 svm_grid_step_C #$# 2 svm_grid_start_G #$# -10 svm_grid_end_G #$# 0 svm_grid_step_G #$# 2 max_kf_devel_pos_set #$# 10000 max_kf_devel_neg_set #$# 20000 max_kf_devel_sub_size #$# 3000 max_kf_shot_devel_neg #$# 1 max_kf_shot_devel_pos #$# 5 neg_sampling_rate #$# 1.0 pos_sampling_rate #$# 1.0 svm_train_mem_size #$# 2000
 */

// general config for hlf-tv20ZZ.cfg
/*
 * exp_name #$# hlf-tv2009 #$# runs using tv2009 dataset exp_dir #$# /net/sfv215/export/raid6/trecvid/experiments #$# full path = exp_dir/exp_name run_list #$# hlf-tv2009.run #$# list of runs concept_list #$# hlf-tv2009.Concepts #$# ~/annotation, concept list used for this experiment devel_pat #$# tv2009.devel-nist.lst #$# ~metadata/tv2009.devel-nist.lst, use NIST keyframes and annotations. However this might be overriden in run config (NEW!) test_pat #$# tv2009.test.lst #$# metadata/trecvid.video.tv2009.test.lst, use own test keyframes. However this might be overriden in run config (NEW!) eval_qrel #$# tv2009.feature.qrels.txt #$# groundtruth file for evaluation, located in metadata dir ann_dir #$# annotation #$# annotation data copied from trecvid/metadata/annotation/concept/ksc ann_dir #$# annotation #$# annotation data copied from trecvid/metadata/annotation/concept/ksc metadata_dir #$# metadata #$# metadata data, e.g. video list for devel and test partitions, copied from trecvid/metadata/keyframe-5 model_dir #$# models #$# models for concept detectors, generated by training process result_dir #$# results #$# prediction results for test dataset, generated by testing process root_metadata_kf_dir #$# /net/sfv215/export/raid6/trecvid/metadata/keyframe-5 #$# common metadata of the archive, used for retrieving lists of keyframes of video programs root_feature_dir #$# /net/sfv215/export/raid6/trecvid/feature/keyframe-5 #$# common feature dir, used for retrieving feature vectors of keyframes
 */
?>