...
if($argc!=4)
{
	...
	printf("Usage %s <RootInDir> <GTFile> <Filter>", $argv[0]);
	exit();
}
...
$szPrjName = "mediaeval-vsd-2014.devel2014-new.test2014-new";
$szProjectKF = "keyframe-5";
$szGroupSet = "test2014-new";
...
$szouttemp = "/net/per610a/export/das11f/ledduy/mediaeval-vsd-2014/code/temp/lqvuLog/CheckResultsETF";
$szStartTime = date("m.d.Y_H.i.s");
$strtmp = sprintf("%s_%s",$szouttemp,$szStartTime);
$szouttemp = $strtmp;
$sztmpOutput = sprintf("%s/CheckResults",$szouttemp);

$szRootLogs = sprintf("%s/CheckResults",$szouttemp);
if (!file_exists($szRootLogs))
{
	makeDir($szRootLogs);
}
$szRootOutput = $szRootLogs;

$szLogAllMAP = sprintf("%s/LogAllMap2014All.csv",$szouttemp);
...
$szResultDir =  sprintf("%s",$szRootDir);
$arrSubFiles = collectFilesInOneDir($szResultDir,$szFilter,".txt");
//$nFeatures = loadListFile($arrSubFiles,$szListFeatures);
sort($arrSubFiles);
foreach ($arrSubFiles as $szRunETF)
{
			
			//Get mAP by Mediaeval
			$szOutfile = sprintf("%s/%s.out",$szRootTmpMAP,$szRunETF);
			$szOutfileCmd = sprintf("%s/cmd_%s.txt",$szRootTmpMAP,$szRunETF);
			
			$szFileETF = sprintf("%s/%s.txt",$szResultDir,$szRunETF);
			
			$rootMAP2014 = GetMAP($szOutfile,$szFileETF,$szGTFile,$szOutfileCmd);
			$rootMAP100 =  ReadMAP100file($szOutfile);
			
			$strW = sprintf("%s,%0.6f,%0.6f\n",$szRunETF,$rootMAP2014,$rootMAP100);
